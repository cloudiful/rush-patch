...
fn parallel_extraction_preserves_input_file_order() {
    let root = temp_root("json_order");
    let data_dir = root.join("www").join("data");
    fs::create_dir_all(&data_dir).expect("create data dir");
    let first = data_dir.join("ZLast.json");
    let second = data_dir.join("AFirst.json");
    fs::write(&first, r#"{"name":"First input"}"#).expect("write first");
    fs::write(&second, r#"{"name":"Second input"}"#).expect("write second");
